  ...
  const fieldPolicies = {
    ...
    experienceYears: { policy: "needsApproval", label: "Years of Experience" },
        ...
        if (user && isEditing && !dataLoaded) {
          ...
          if (therapist) {
            form.reset({
              ...
              registrationNo: therapist.registrationNo, // Mock data
              experienceYears: therapist.experience || 0,
  ...
  //   function onSubmit(data: TherapistOnboardingValues) {
  //                 console.log('Create therapist data:', data);

  //       startTransition(async () => {
  //           if(isEditing) {
  //             const result = await requestProfileUpdate(data);
  //              if (result.success) {
  //                 toast({
  ...
  //             }
  //         } else {
  //             const result = await createTherapistAction(data);
  //             if(result.success) {
  //                 toast({
  //                     title: 'Registration Successful!',
  //                     description: 'Your profile has been submitted for verification. Please sign in.',
  //                 });
...
console.log("TherapistOnboardingValues",data);
    try {
      let result;

      if (isEditing) {
                    console.log('Update therapist data new:', data); // ye ab print hoga

        result = await requestProfileUpdate(data);
      } else {

        result = await createTherapistAction(data);
      }

      console.log('Server result:', result); // ye bhi print hoga
      ...
      console.error('Submission error:', error); // ye ab errors bhi show karega
    }
  }
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
          <Tabs defaultValue="account">
                  ...
                  <CardContent className="space-y-4">
                      ...
                      <div className="space-y-4">
                          <div className="grid sm:grid-cols-2 gap-6">
                              <FormFieldWrapper fieldName="line1"><FormField name="line1" control={form.control} render={({ field }) => (<FormItem><FormLabel>Address Line 1</FormLabel><FormControl><Input placeholder="Building name and street" {...field} /></FormControl><FormMessage /></FormItem>)}/></FormFieldWrapper>
              ...
              <TabsContent value="professional" className="space-y-6">
                  ...
                  <CardContent className="space-y-4">
                      ...
                      <div className="grid sm:grid-cols-1 md:grid-cols-2 gap-6">
                          <FormFieldWrapper fieldName="kycIdProof"><FormField name="kycIdProof" control={form.control} render={({ field: { onChange, ...fieldProps} }) => (<FormItem><FormLabel>ID Proof Document</FormLabel><FormControl><Input type="file" onChange={(e) => onChange(e.target.files)} {...fieldProps} /></FormControl><FormDescription>Aadhar, PAN Card, etc.</FormDescription><FormMessage /></FormItem>)}/></FormFieldWrapper>
                          <FormFieldWrapper fieldName="kycLicense"><FormField name="kycLicense" control={form.control} render={({ field: { onChange, ...fieldProps} }) => (<FormItem><FormLabel>License Document</FormLabel><FormControl><Input type="file" onChange={(e) => onChange(e.target.files)} {...fieldProps} /></FormControl><FormDescription>Professional license</FormDescription><FormMessage /></FormItem>)}/></FormFieldWrapper>
                      </div>
              ...
              <TabsContent value="availability" className="space-y-6">
                  ...
                  <CardContent className="grid md:grid-cols-2 gap-6">
                      <FormFieldWrapper fieldName="experienceYears"><FormField name="experienceYears" control={form.control} render={({ field }) => (<FormItem><FormLabel>Years of Experience</FormLabel><FormControl><Input type="number" placeholder="5" {...field} /></FormControl><FormMessage /></FormItem>)}/></FormFieldWrapper>
                  ...
                  <CardContent className="space-y-4">
                      ...
                      {days.map((day) => (
                          <div key={day} className="p-4 border rounded-lg grid md:grid-cols-3 gap-4 items-start">
                              <Controller name={`availability.${day}.enabled`} control={form.control} render={({ field }) => (
                              ...
                              <div className="space-y-1"><Label>Morning (06:00 - 13:00)</Label><div className="flex gap-2"><FormField name={`availability.${day}.morning.start`} control={form.control} render={({ field }) => (<FormItem><TimeSlotSelect field={field} slots={morningSlots} disabled={!form.watch(`availability.${day}.enabled`)} /></FormItem>)}/> <FormField name={`availability.${day}.morning.end`} control={form.control} render={({ field }) => (<FormItem><TimeSlotSelect field={field} slots={morningSlots} disabled={!form.watch(`availability.${day}.enabled`)} /></FormItem>)}/></div></div>
                              <div className="space-y-1"><Label>Evening (13:00 - 21:00)</Label><div className="flex gap-2"><FormField name={`availability.${day}.evening.start`} control={form.control} render={({ field }) => (<FormItem><TimeSlotSelect field={field} slots={eveningSlots} disabled={!form.watch(`availability.${day}.enabled`)} /></FormItem>)}/> <FormField name={`availability.${day}.evening.end`} control={form.control} render={({ field }) => (<FormItem><TimeSlotSelect field={field} slots={eveningSlots} disabled={!form.watch(`availability.${day}.enabled`)} /></FormItem>)}/></div></div>
                          </div>
                      ))}
                  </CardContent></Card>
              </TabsContent>
              ...
              <TabsContent value="payouts" className="space-y-6">
                  ...
                  <CardContent className="space-y-4">
                      ...
                      <FormFieldWrapper fieldName="kycBankProof"><FormField name="kycBankProof" control={form.control} render={({ field: { onChange, ...fieldProps} }) => (<FormItem><FormLabel>Bank Proof Document</FormLabel><FormControl><Input type="file" onChange={(e) => onChange(e.target.files)} {...fieldProps} /></FormControl><FormDescription>Canceled cheque, statement</FormDescription><FormMessage /></FormItem>)}/></FormFieldWrapper>
                  </CardContent></Card>
              </TabsContent>
          </Tabs>
