...
const therapyNavItems = (activeAlertCount: number, openTicketCount: number, pendingApprovalsCount: number) => [
    ...
    { type: 'group', label: 'Team & Governance', icon: Users, items: [
        ...
        { href: '/dashboard/therapy-admin/profile-approvals', label: 'Profile Approvals', icon: UserCheck, badgeCount: pendingApprovalsCount },
...
export function TherapyAdminNav() {
    ...
    React.useEffect(() => {
        const fetchData = async () => {
            ...
            setPendingApprovalsCount(approvals.filter(r => r.status === 'pending' && r.role === 'therapist').length);
        }
        fetchData();
    }, []);
    ...
    const navigationItems = therapyNavItems(activeAlertCount, openTicketCount, pendingApprovalsCount);
