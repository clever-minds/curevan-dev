...
const ActionsMenu = ({ order, scope, asSheetItems = false }: { order: Order, scope: OrdersTableProps['scope'], asSheetItems?: boolean }) => {
  const { toast } = useToast();
  const isAdmin = scope === 'admin' || scope === 'ecom-admin';
  const isPatient = scope === 'patient';
  const hasInvoice = order.status !== 'Placed' && order.status !== 'Cancelled';
  ...
  const content = (
    <>
      {hasInvoice && <DropdownMenuItem asChild><Link href={`/dashboard/invoices?id=${order.invoiceId}`} className="flex items-center w-full"><FileText className="mr-2"/>View Invoice</Link></DropdownMenuItem>}
      {order.status === 'Shipped' || order.status === 'Delivered' && (
          <DropdownMenuItem><Truck className="mr-2"/>Track Shipment</DropdownMenuItem>
      )}
      {isPatient && order.status === 'Delivered' && (
          <Dialog>
              <DialogTrigger asChild>
                  <DropdownMenuItem onSelect={(e) => e.preventDefault()}><Star className="mr-2"/>Leave Feedback</DropdownMenuItem>
              </DialogTrigger>
              <DialogContent>
                  <DialogHeader>
                  ...
                  <SupportFeedbackForm formType="feedback" feedbackTopic="product" feedbackItemId={order.id} />
              </DialogContent>
          </Dialog>
      )}
      {isPatient && isReturnable() && <DropdownMenuItem><Undo className="mr-2"/>Return/Replace Items</DropdownMenuItem>}

      {isAdmin && (
          <>
            <DropdownMenuSeparator />
            <DropdownMenuItem><Info className="mr-2" />View Order Details</DropdownMenuItem>
            <DropdownMenuItem><Printer className="mr-2" />Print Packing Slip</DropdownMenuItem>
            <DropdownMenuItem onClick={handleCreateShipment} disabled={order.status === 'Shipped' || order.status === 'Delivered' || isCreatingShipment}>
                {isCreatingShipment ? <Loader2 className="mr-2 animate-spin" /> : <Truck className="mr-2" />}
                Create Shipment
            </DropdownMenuItem>
            <DropdownMenuSeparator />
            <DropdownMenuItem className="text-red-600 focus:text-red-700"><Undo className="mr-2" />Refund</DropdownMenuItem>
            <DropdownMenuItem className="text-destructive focus:text-destructive"><Ban className="mr-2" />Cancel Order</DropdownMenuItem>
          </>
      )}
    </>
  );
      ...
      <Sheet>
        ...
        <SheetContent side="bottom" className="h-auto">
          ...
          <div className="py-4 space-y-2">
             {hasInvoice && <Button variant="outline" className="w-full justify-start" asChild><Link href={`/dashboard/invoices?id=${order.invoiceId}`}><FileText className="mr-2"/>View Invoice</Link></Button>}
             ...
             {isAdmin && <Button variant="destructive" className="w-full justify-start"><Ban className="mr-2" />Cancel Order</Button>}
          </div>
        </SheetContent>
      </Sheet>
    )
        ...
        <Card>
            <CardContent className="p-4">
                ...
                <div className="mt-4 flex justify-end">
                    <ActionsMenu order={order} scope={scope} asSheetItems />
                </div>
            </CardContent>
        </Card>
    );
              ...
              {orders.map((order) => {
                ...
                <TableRow key={order.id}>
                  ...
                  <TableCell className="text-right">
                    <ActionsMenu order={order} scope={scope} />
                  </TableCell>
                </TableRow>
                )
              })}
