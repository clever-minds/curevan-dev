...
const mapProduct = (p: any): Product => ({
  ...
  subCategoryId: p.sub_category_id ? Number(p.sub_category_id) : undefined,
  subCategoryName: p.sub_category_name || undefined,
  brand: p.brand || undefined,
  discountPercentage: p.mrp && p.sellingPrice ? Math.round(((p.mrp - p.sellingPrice) / p.mrp) * 100) : (p.mrp && p.selling_price ? Math.round(((p.mrp - p.selling_price) / p.mrp) * 100) : 0),
